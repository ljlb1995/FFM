...
class Subgrupo extends ActiveRecord {
    ...
    */
    ...
    public function getInformacionSubgrupo($id, $isSlug=false) {
        $id = ($isSlug) ? Filter::get($id, 'string') : Filter::get($id, 'numeric');
        $columnas = 'subgrupo.*';
        $join = '';
        $condicion ="subgrupo.id = '$id'";
     ...
     * @return type
     */
    public function getListadoSubgrupos($estado='todos', $order='order.nombre.asc', $page=0) {                           
        $columns = 'subgrupo.*';

     
          $order = $this->get_order($order, 'nombre', array('subgrupo'=>array('ASC'=>'subgrupo.nombre ASC, subgrupo.descripcion ASC','DESC'=>'subgrupo.nombre DESC, subgrupo.descripcion DESC'),'descripcion'));
        if($page) {            
    ...
    public function getListadoSelect($estado='todos', $order='order.nombre.asc', $page=0) {                           
        $columns = 'subgrupo.*';
          ...
          $order = $this->get_order($order, 'nombre', array('subgrupo'=>array('ASC'=>'subgrupo.id ASC, subgrupo.nombre ASC','DESC'=>'subgrupo.id DESC, subgrupo.nombre DESC')));
        if($page) {            
            return $this->paginated("columns: $columns",  "order: $order", "page: $page");
        }
        return $this->find("columns: $columns", "order: $order");
     ...
     */
    public static function setSubgrupo($method, $data, $optData=null) {        
        $obj = new Subgrupo($data); //Se carga los datos con los de las tablas        
        ...
        if($check) { //Si existe
            if(empty($obj->id)) {
                ...
                if($obj->id != $old->id) {
                    DwMessage::info('Lo sentimos, pero ya existe un subgrupo registrado con el mismo número de identificación');
                    return FALSE;
                }
            }
    ...
    public function before_save() {
       $this->nombre = Filter::get($this->nombre, 'string');
        ...
        if($this->count("conditions: $conditions")) {
            DwMessage::error('Lo sentimos, pero ya existe un subgrupo registrado con el mismo nombre.');
            return 'cancel';
        }
    ...
    public function before_delete() {
        ...
        if($this->modulo=='sistema' OR ($this->Subgrupos == '*' && empty($this->controlador)) OR ($this->Subgrupos=='principal/*') ) {
            return 'cancel';
        }
    }
    
     public function getAjaxSubgrupos($field, $value, $order='', $page=0) {
        ...
        $order = $this->get_order($order, 'nombre', array(                        
            'nombre' => array(
                'ASC'=>'subgrupo.nombre ASC, subgrupo.nombre ASC', 
                'DESC'=>'subgrupo.nombre DESC, subgrupo.nombre DESC'
            ),
