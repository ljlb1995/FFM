...
public function buscar(){
    return $this->find('order: nombreEstado');
  }

    public function getInformacionEstado($id, $isSlug=false) {
        $id = ($isSlug) ? Filter::get($id, 'string') : Filter::get($id, 'numeric');
        $columnas = 'estado.*';
        $join = '';
        $condicion ="estado.id = '$id'";
     ...
     * @return type
     */
    public function getListadoEstado($estado='todos', $order='order.nombreEstado.asc', $page=0) {                           
